...
: gray0  ( -- length )                  \ gray scale followed by 0s
   /src  src @ if                       \ format: 0110gggg_cccccccc  gray, 0s
      src@ 8 lshift $6000 or y !
...
: gray1  ( -- length )                  \ gray scale followed by 1s
   /src  src @ if                       \ format: 0111gggg_cccccccc  gray, Fs
      src@ 8 lshift $7000 or y !
      src@ 1 15 within if               \ starts with gray
         bump  127 runF
      then
   then  score  dup 1- y +!
   ...
   |usedchars| 0 ?do
      i [usedchars] @ if
        ...
        fheight if  compress
           ...
           left @ c,  top @ c,
           fwidth c,  fheight c,
           compbuf wcount  dup w,f  0 ?do wcount w,f loop  drop
           ." ."
        else
...
\ Revision number of the font (24-bit)
...
: @f  ( a -- n )
   count 16 lshift >r
   count  8 lshift >r                   \ fetch 24-bit number
   c@  r> +  r> +
;
: n!  ( n a -- )                        \ store 24-bit number
   over 16 rshift over c!  1+
   over  8 rshift over c!  1+
   c!
;
: w!f  ( n a -- )                       \ store 16-bit number
   over  8 rshift over c!  1+
   c!
;
: w@f  ( a -- n )
   count  8 lshift >r                   \ fetch 16-bit number
   c@  r> +
;
: fhere  ( -- n ) here FontHome - ;     \ relative HERE

: /FONTS  ( revision n -- )             \ set the number of fonts being compiled
   here to FontHome  dup c,  swap n,
   3 * 0 ?do 0 c, loop
;
: ,fine  ( idx -- zero? )
...
: MakeTable  ( fontID -- )              \ populate glyph tables for this font
   dup FontID !
   fhere  swap 3 *  FontHome 4 + + n!   \ font[fontID] -> here (the coarse table)
   here 'coarse !
   ...
   0 ?do
      ...
      i 2* 'coarse @ +  w!f             \ resolve pointer to fine table
   loop
;
...
variable tally
variable ddims                          \ width:height
create hexdigs ," .123456789ABCDE#"
: .pix  ( n -- )                        \ output a pixel
   tally @  ddims 1+ c@  mod 0= if cr then
...
: pint  ( u16 -- )                      \ generate a run of pixels from command
   ...
   then
   dup $F000 and $6000 = if             \ format: 0110gggg_cccccccc  gray, 0s
      $FFF and $100 /mod  .pix
      0 ?do  0 .pix  loop  exit
   then
   dup $F000 and $7000 = if             \ format: 0111gggg_cccccccc  gray, Fs
      $FFF and $100 /mod  .pix
      0 ?do 15 .pix  loop  exit
   then
   drop ." ?"
...
: dum  ( -- )                           \ dump compressed data in hex
   compbuf wcountf  cr dup . ." words"
   0 ?do  wcountf i @ 7 and  if cr then  h.
   loop  drop
;
...
: faddr  ( xchar -- addr | 0 )
   ...
   FontID @ 3* +  @f  FontHome +        \ 'tables for this FontID
